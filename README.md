...
### 11. 简单处理器 sCPU 微架构核心部件 (F5 阶段)
* **`ROM_16MUL8`（指令存储器与取指通路，Instruction ROM & Fetch Unit）**：
  * 基于 16 选 1 的 8 位宽 MUX 实现，内部预存数列求和程序 8 条机器码（`0x8a`, `0x90`, `0xa0`, `0xb1`, `0x17`, `0x29`, `0xd1`, `0xdf`）；
  * 配合 8 位 PC 寄存器与加法器构成自增与跳转选通数据通路，实现精准无延迟的逐拍取指。
* **`RAM_16MUL8`（通用寄存器堆，General Purpose Registers / RAM）**：
  * 采用工业级标准微架构：写数据总线（`wdata`）全并联广播 + 写地址（`waddr`）经 3-8 译码器进行写使能（`EN`）门控，彻底杜绝非选中单元被误抹为 0；
  * 8 单元 × 8 位宽独立数据寄存器阵列，8 选 1 读 MUX 实时直读（`rdata`），接口完全独立解耦，向下兼容 sISA 4 个通用寄存器（`r0 ~ r3`）规范。
