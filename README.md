# 一生一芯·F3~F6 数字逻辑、简单处理器与外设微架构全栈实现 (OSCPU Digital Circuits, sCPU & I/O Subsystem)
...
🎉 **里程碑达成：F3 阶段数字逻辑电路基础、F5 阶段 sCPU 简单处理器、以及 F6 阶段外设人机交互系统圆满收官（F3 ~ F6 100% Completed）！**
...
* **F6 阶段（外设系统集成与人机交互闭环，新增 `sCPU_li_add`、`PC` 及 I/O 子系统）**：
  * **统一编址 I/O 外设接口**：集成 8 位拨码开关（`dev=000`）、8 键按键矩阵（`dev=001`）、8 位 LED 灯排（`dev=000`）、双位十六进制七段数码管（`dev=001`）；
  * **高可靠性硬件总线隔离**：反相器输入方向控制门控（`is_io & !dir`）、位扩展掩码与门总线保护、3-8 独立外设译码器（彻底消灭外设互相抹零）；
  * **解决广播雪崩的工业级 Tie-off**：输入 MUX 闲置通道全面下拉接地锁定（Tie-to-GND），消除高阻态击穿；
  * **软件生态全谱跑通**：从拨码开关动态读入末项求和、N=0 边界 Bug 实证剖析（模 256 溢出回绕至 128 / `0x80`），到按键轮询（Polling）触发数码管呈现的完整交互式程序。
...
## 📚 长期维护的工程设计与调试全纪录
> **👉 [点击查阅：F3~F6 全程设计与调试实战演进日志 (F3_CIRCUIT_DESIGN_AND_DEBUG_LOG.md)](./F3_CIRCUIT_DESIGN_AND_DEBUG_LOG.md)**  
> 详细记录全阶段 12 个章节的**设计动机、踩坑现象、假设验证、底层物理成因（Root Cause）、架构演进与体系结构哲学**：
...
> 12. **F6 外设系统与人机交互微架构：输入输出总线隔离、反相器使能门控、按键轮询等待（Polling Loop）与工业级 Tie-off 防御哲学**。
...
### 12. 具备外设交互与高级指令集的单周期处理器 (F6 阶段终极集大成)
* **`sCPU_li_add`（完备人机交互单周期处理器顶层，Full Single-Cycle CPU with I/O）**：
  * **4 类指令集完备支持**：立即数加载 `li`（2 位分片槽号）、算术加法 `add`、外部输入输出 `io`、以及基于相对偏移量（PC-Relative Offset）的条件跳转 `bner0`；
  * **模块化程序计数器 `PC`**：内置 PC 自增数据通路、跳转目标数据输入（`datain`）、跳转选通（`ctr`）与全局同步清零（`CLR`）；
  * **分支跳转流水线**：4 位加法器计算 $PC_{next} = PC + \text{offset}$，配合 8 位比较器检测 $rdata2 \neq r0$ 动态装载；
  * **人机交互控制**：支持拨码开关输入末项、按键轮询触发、LED 实时呈现中间项、数码管稳定呈现最终计算结果。

---

## 📜 迄今为止全谱机器码程序字典 (Programs 1 ~ 5)

| 编号 | 程序名称与应用场景 | 运行平台 | 指令长度 | 核心机器码序列 (Hex) |
| :---: | :--- | :---: | :---: | :--- |
| **P1** | **F5 绝对跳转经典求和** ($1+2+\dots+10=55$) | `sCPU_full` | 8 字节 | `8a 90 a0 b1 17 29 d1 df` |
| **P2** | **F6 槽位立即数与相对跳转基准求和** | `sCPU_li_add` | 10 字节 | `86 92 01 91 a0 b0 29 3e fa c3` |
| **P3** | **F6 拨码开关直通数码管透传测试** | `sCPU_li_add` | 3 字节 | `50 59 c1` |
| **P4** | **F6 拨码动态求和与 N=0 边界 Bug 测试** | `sCPU_li_add` | 10 字节 | `40 91 a0 b0 29 3e fa 78 79 c3` |
| **P5** | **F6 按键交互式求和** (LED当前项+按键轮询+数码管触发) | `sCPU_li_add` | 16 字节 | `86 92 01 91 a0 b0 29 68 3e f6 80 61 ca f9 79 c1` |
